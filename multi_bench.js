...
Test.prototype.run = function (callback) {
    ...
    for (i = 0; i < num_clients ; i++) {
        this.clients.push(redis.createClient());
    }

    var promises = this.clients.map(function (client) {
        return client.connect();
    });

    var self = this;
    return rsvp.all(promises).then(function () {
        self.clients_ready = num_clients;
...
Test.prototype.stop_clients = function () {
    ...
    return rsvp.all(promises).then(this.callback);
};
...
Test.prototype.send_next = function () {
    ...
    this.clients[cur_client].send(this.args.command, this.args.args).then(function (res) {
